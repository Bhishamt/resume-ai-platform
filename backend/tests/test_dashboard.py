...
def test_dashboard_preferences_service(db_session):
    ...
    DashboardService.update_user_preferences(
        db_session, user_id, layout=new_layout, theme="light"
    )
...
def test_api_update_preferences(client, db_session, auth_headers):
    ...
    response = client.put(
        "/api/v1/dashboard/preferences", json=payload, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
