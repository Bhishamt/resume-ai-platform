...
def create_job_description(db: Session, user_id: UUID, obj_in: JobDescriptionCreate) -> JobDescription:
...
def get_job_description(db: Session, jd_id: UUID, user_id: UUID) -> JobDescription:
    ...
    if jd.user_id != user_id:
        raise AuthorizationError("You do not have permission to access this job description.")
    return jd
...
def run_job_match(
    ...
    if job_description_id:
        ...
        raise BadRequestError("Must provide either an existing job_description_id or raw job_description details.")
    ...
    if not parsed_text or not parsed_text.strip():
        raise BadRequestError("Unable to match. The selected resume contains no readable text content.")
...
def get_job_match(db: Session, match_id: UUID, user_id: UUID) -> JobMatch:
    ...
    if match.resume.user_id != user_id:
        raise AuthorizationError("You do not have permission to access this match report.")
    return match

def get_user_job_matches(db: Session, user_id: UUID, skip: int = 0, limit: int = 100) -> list[JobMatch]:
