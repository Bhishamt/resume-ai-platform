    ...
    def analyze_keywords(cls, text: str) -> dict:
        ...
        for kw in cls.CRITICAL_KEYWORDS:
            ...
            if kw == "api" and not kw_found:
                for cat_list in found_by_category.values():
                    if any(
                        "rest api" in x.lower() or "api" in x.lower() for x in cat_list
                    ):
                        kw_found = True
                        break
        ...
        for kw in cls.CRITICAL_KEYWORDS:
            ...
            if not kw_found:
                ...
                reasons.append(
                    {"rule": f"Missing {display_name} keyword", "points": deduction}
                )
        ...
        if tech_count < 5:
            reasons.append(
                {"rule": "Low density of technical skills (<5 skills)", "points": -4}
            )
            weighted_score -= 4
        elif tech_count < 10:
            reasons.append(
                {
                    "rule": "Moderate density of technical skills (5-9 skills)",
                    "points": -2,
                }
            )
            weighted_score -= 2
        ...
        if soft_count < 3:
            reasons.append(
                {"rule": "Low density of soft skills (<3 skills)", "points": -3}
            )
            weighted_score -= 3
        elif soft_count < 5:
            reasons.append(
                {"rule": "Moderate density of soft skills (3-4 skills)", "points": -1}
            )
            weighted_score -= 1

        if verbs_count < 4:
            reasons.append(
                {"rule": "Low count of action verbs (<4 verbs)", "points": -3}
            )
            weighted_score -= 3
        elif verbs_count < 8:
            reasons.append(
                {"rule": "Moderate count of action verbs (4-7 verbs)", "points": -1}
            )
            weighted_score -= 1
        else:
        ...
        if ind_count < 3:
            reasons.append(
                {"rule": "Low density of industry keywords (<3 keywords)", "points": -3}
            )
            weighted_score -= 3
        elif ind_count < 5:
            reasons.append(
                {
                    "rule": "Moderate density of industry keywords (3-4 keywords)",
                    "points": -1,
                }
            )
            weighted_score -= 1
