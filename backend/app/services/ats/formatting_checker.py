...
class FormattingChecker:
    ...
    def check_formatting(cls, text: str, detected_sections: list) -> dict:
        ...
        if 300 <= word_count <= 1500:
            ...
            reasons.append(
                {
                    "rule": "Word count slightly outside optimal 300-1500 range",
                    "points": -1,
                }
            )
            weighted_score -= 1
            feedback.append(
                "Resume word count is slightly outside the optimal 300-1500 range."
            )
        else:
            reasons.append(
                {
                    "rule": "Word count is way too short or long (<150 or >2000)",
                    "points": -3,
                }
            )
            weighted_score -= 3
            feedback.append(
                "Resume word count is either too short or too long. Aim for 300-1500 words."
            )
        ...
        if section_count >= 4:
            ...
            reasons.append(
                {"rule": "Fewer than 4 standard sections detected", "points": -1}
            )
            weighted_score -= 1
            feedback.append(
                "Resume does not have enough clearly defined sections. Include Skills, Experience, Education, and Projects."
            )
        else:
            reasons.append({"rule": "No standard sections detected", "points": -3})
            weighted_score -= 3
            feedback.append(
                "No standard section headers detected. Check your resume's heading formatting."
            )
        ...
        if bullet_count >= 8:
            ...
            reasons.append(
                {"rule": "Fewer than 8 bullet points detected", "points": -1}
            )
            weighted_score -= 1
            feedback.append(
                "Increase bullet point usage to make achievements easier to read."
            )
        else:
            reasons.append(
                {"rule": "No bullet points used to structure content", "points": -3}
            )
            weighted_score -= 3
            feedback.append(
                "No bullet points detected. Structure your experience and projects with bulleted lists."
            )
        ...
        if lines:
            for line in lines[:3]:
                if any(
                    kw in line.lower()
                    for kw in ["resume", "cv", "curriculum", "email", "phone"]
                ):
                    continue
                words_in_line = line.split()
                if 1 <= len(words_in_line) <= 4 and all(
                    w[0].isupper() for w in words_in_line if w.isalpha()
                ):
                    has_name = True
                    break
            if not has_name:
        ...
        if not has_name:
            reasons.append(
                {
                    "rule": "Candidate name could not be identified at the top",
                    "points": -2,
                }
            )
            weighted_score -= 2
            feedback.append(
                "Candidate name could not be identified at the top of the resume."
            )
        ...
        if not has_email:
            ...
            weighted_score -= 2
            feedback.append(
                "No email address detected. Ensure your email is clearly visible."
            )
