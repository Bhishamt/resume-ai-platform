...
class TrendService:
    ...
    def get_user_trends(db: Session, user_id: UUID) -> Dict[str, Any]:
        ...
        resumes = db.query(Resume.upload_date).filter(
            Resume.user_id == user_id,
            Resume.upload_status == "success"
        ).order_by(Resume.upload_date).all()
        ...
        upload_timeline = [
            {"date": d, "count": c}
            for d, c in sorted(upload_counts.items())
        ]

        # 3. Job Match trend
        matches = db.query(JobMatch).join(Resume).filter(
            Resume.user_id == user_id
        ).order_by(JobMatch.created_at).all()

        job_match_trend = [
            {
                "date": m.created_at.strftime("%Y-%m-%d"),
                "score": m.overall_match,
                "company": m.job_description.company if m.job_description else "Unknown",
