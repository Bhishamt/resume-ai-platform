...
class AIService:
    ...
    async def _execute_ai_task(
        ...
        cached_result = ai_cache.get(
            prompt_text, self.provider.__class__.__name__, prompt_type
        )
        if cached_result:
            logger.info(
                f"Cache hit for prompt type '{prompt_type}' and provider '{self.provider.__class__.__name__}'"
            )
            return cached_result

        # 4. Generate AI response
        ai_data = await self.provider.generate_response(
            prompt=prompt_text, system_prompt=system_prompt, json_mode=True
        )
        ...
        # 8. Cache response
        ai_cache.set(
            prompt_text, self.provider.__class__.__name__, prompt_type, parsed_content
        )
    ...
    async def review_resume(
        ...
        if analysis_id:
            ...
            analysis = (
                db.query(AnalysisReport)
                .filter(AnalysisReport.resume_id == resume_id)
    ...
    async def improve_summary(
        self, db: Session, user_id: UUID, resume_id: UUID
    ) -> Dict[str, Any]:
        """
        ...
        variables = {"resume_text": resume.parsed_text or ""}
    ...
    async def improve_projects(
        ...
        return {
            "better_project_descriptions": rewrite_result.get(
    ...
    async def prepare_interview(
        ...
        variables = {"resume_text": resume.parsed_text or "", "job_text": job_text}
    ...
    def delete_history_item(
        self, db: Session, user_id: UUID, feedback_id: UUID
    ) -> bool:
        """
        ...
        feedback = (
            db.query(AIFeedback)
            .filter(AIFeedback.id == feedback_id, AIFeedback.user_id == user_id)
            .first()
        )
        if not feedback:
            return False
