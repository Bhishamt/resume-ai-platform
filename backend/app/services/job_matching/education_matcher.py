...
class EducationMatcher:
    ...
    def _get_degree_level(cls, text: str) -> tuple[int, str]:
        ...
        if any(x in text_lower for x in ["phd", "ph.d", "doctorate"]):
            return 4, "PhD"
        if any(x in text_lower for x in ["master", "m.s.", "m.s", "ms", "mba", "m.tech"]):
            return 3, "Master's"
        if any(x in text_lower for x in ["bachelor", "b.s.", "b.s", "bs", "b.tech", "b.a."]):
            return 2, "Bachelor's"
    ...
    def match_education(cls, resume_text: str, education_requirement: str) -> Dict:
        ...
        if not education_requirement:
            reasons.append({"rule": "No educational requirement specified by job", "points": 0.0})
            return {
                "score": 100,
                "weighted_score": 10.0,
                "reasons": reasons
        ...
        if cand_level >= req_level:
            reasons.append({"rule": f"Degree requirement met: Required {req_name}, candidate has {cand_name}", "points": 0.0})
        else:
            diff = req_level - cand_level
            deduction = 4.0 if diff == 1 else 6.0
            reasons.append({"rule": f"Degree mismatch: Required {req_name} but highest found is {cand_name}", "points": -deduction})
            weighted_score -= deduction

        # 2. Check Discipline / Major
        disciplines = ["computer science", "software engineering", "information technology", "mathematics", "business", "engineering", "finance", "data science"]
        ...
        if required_disciplines:
            ...
            if matched_discipline:
                reasons.append({"rule": "Discipline/Field of study matched", "points": 0.0})
            else:
                reasons.append({"rule": f"Discipline mismatch: Required field ({', '.join(required_disciplines)}) not found in resume", "points": -4.0})
                weighted_score -= 4.0
