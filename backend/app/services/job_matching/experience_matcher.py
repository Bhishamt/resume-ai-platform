...
class ExperienceMatcher:
    ...
    def match_experience(
        cls, resume_text: str, required_experience: str, job_title: str
    ) -> Dict:
        ...
        if candidate_years == 0:
            exp_statement = re.search(
                r"\b(\d+)\+?\s*years?\s+of\s+experience\b", resume_text.lower()
            )
            if exp_statement:
        ...
        if candidate_years >= req_years:
            reasons.append(
                {
                    "rule": f"Meets or exceeds required experience of {req_years} years (+{candidate_years} years found)",
                    "points": 0.0,
                }
            )
        else:
            ...
            reasons.append(
                {
                    "rule": f"Missing {missing_years} years of required experience",
        ...
        if required_seniority:
            ...
            if has_seniority:
                reasons.append(
                    {"rule": "Seniority keyword alignment matched", "points": 0.0}
                )
            else:
                reasons.append(
                    {
                        "rule": "Seniority mismatch: Job requires senior-level titles but resume lacks them",
                        "points": -4.0,
                    }
                )
                weighted_score -= 4.0
        ...
        if found_job_domains:
            ...
            if domain_match:
                ...
                reasons.append(
                    {
                        "rule": f"Role mismatch: Missing domain keywords ({', '.join(found_job_domains)}) in resume",
                        "points": -6.0,
                    }
                )
                weighted_score -= 6.0
