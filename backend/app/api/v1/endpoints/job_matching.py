...
@router.get(
    "",
    response_model=APIResponse,
    summary="Get user's job match reports"
)
def get_matches(
    skip: int = 0,
    limit: int = 100,
    ...
    return APIResponse(
        ...
        data=[JobMatchResponse.model_validate(r).model_dump(mode="json") for r in reports]
    )

@router.get(
    "/{id}",
    response_model=APIResponse,
    summary="Get job match report details by ID"
)
def get_match(
    id: UUID,
    current_user: User = Depends(get_current_active_user),
...
@router.delete(
    "/{id}",
    response_model=APIResponse,
    summary="Delete job match report by ID"
)
def delete_match(
    id: UUID,
    current_user: User = Depends(get_current_active_user),
    ...
    job_matching_service.delete_job_match(
        db=db,
        match_id=id,
        user_id=current_user.id
    )
