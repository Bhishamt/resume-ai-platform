...
@router.get(
    "",
    response_model=APIResponse,
    summary="Get user's analysis reports"
)
def get_analyses(
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
    ...
    return APIResponse(
        ...
        data=[AnalysisReportResponse.model_validate(r).model_dump(mode="json") for r in reports]
    )

@router.get(
    "/{id}",
    response_model=APIResponse,
    summary="Get analysis report details by ID"
)
def get_analysis(
    id: UUID,
    current_user: User = Depends(get_current_active_user),
